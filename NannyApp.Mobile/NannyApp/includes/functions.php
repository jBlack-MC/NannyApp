...
//  Uploaded media (avatars, portfolio docs, etc.) is stored under
//  SHARED_STORAGE_DIR (NannyApp.Shared/storage/ when available, or a local
//  assets/ folder in standalone mode) and streamed back via api/media.php,
//  so files uploaded from this API are visible to the web app too.
// ----------------------------------------------------------------------

/** Build an absolute URL the Android app can load for a stored image/document path. */
function media_url(?string $path): ?string
{
    if ($path === null || $path === '') {
        return null;
    }
    // Legacy demo/static images that only exist inside the separate web app's
    // assets/ folder aren't reachable from here — nothing sensible to link to.
    if (str_starts_with($path, 'assets/')) {
        return null;
    }
    return storage_url($path, fn(string $p) => api_root_url() . 'media.php?f=' . rawurlencode($p));
}

/** Absolute base URL of this API deployment, e.g. "https://host/nannyapp/api/". */
function api_root_url(): string
{
    ...
    $pos    = strpos($uri, '/api/');
    $base   = $pos !== false ? substr($uri, 0, $pos + 5) : '/api/';
...
function avatar(string $name, ?string $image = null, string $extraClass = ''): string
{
    ...
    if ($image) {
        return '<img class="' . e($cls) . '" src="' . e((string) media_url($image)) . '" alt="' . e($name) . '">';
    }
...
/**
 * Validate & store an uploaded image via the active storage driver (local disk,
 * sharded by month, or S3-compatible object storage in production — see
 * NannyApp.Shared/config/storage.php).
...
function save_uploaded_image(array $file, string $subdir = 'uploads'): array
{
    ...
    $path = storage_new_path($subdir, $ext);
    if (!storage_store_upload($file['tmp_name'], $path)) {
