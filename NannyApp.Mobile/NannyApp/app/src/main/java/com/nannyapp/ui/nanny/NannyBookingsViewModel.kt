...
data class NannyBookingsUiState(
...
@HiltViewModel
class NannyBookingsViewModel @Inject constructor(private val bookingRepository: BookingRepository) : ViewModel() {
    private val _state = MutableStateFlow(NannyBookingsUiState())
    val state: StateFlow<NannyBookingsUiState> = _state.asStateFlow()

    init { load() }

    fun load() {
        viewModelScope.launch {
            bookingRepository.getBookings(UserRole.NANNY).collect { result ->
    ...
    val filteredBookings: List<Booking> get() = _state.value.filter?.let { f -> _state.value.bookings.filter { it.status == f } } ?: _state.value.bookings
}
