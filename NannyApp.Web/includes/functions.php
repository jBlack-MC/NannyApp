...
//  Uploaded media (profile photos, verification docs, etc.) lives in
//  NannyApp.Shared/storage/ so the web app and the packaged app both read
//  the exact same files instead of duplicating them per project. Files
//  are served through media.php rather than a direct static URL, since
//  the shared folder sits outside this project's webroot.
// ----------------------------------------------------------------------

/** Build a browser-facing URL for a stored image/document path. */
...
function avatar(string $name, ?string $image = null, string $extraClass = ''): string
{
    ...
    if ($image) {
        return '<img class="' . e($cls) . '" src="' . e(media_url($image)) . '" alt="' . e($name) . '">';
    }
 ...
 * under NannyApp.Shared/storage/, or S3-compatible object storage in
 * production — see NannyApp.Shared/config/storage.php).
