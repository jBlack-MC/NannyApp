...
| File uploads | Validated `move_uploaded_file()`/S3 via the shared storage driver (local disk or S3-compatible object storage) — see `NannyApp.Shared/config/storage.php` |
...
| `NannyApp.Shared/database/migrate_v4.sql` | admin_profiles table; booking columns for check-in PIN, check-in/out timestamps, parent confirmation, dispute tracking; payment `payout_status` (held/released/refunded) |
...
- `config/database.php` — requires `NannyApp.Shared/config/database.php`, which defines the `DB_*` constants and the PDO singleton `db()` shared by every Nanny-App client
- `includes/functions.php` — shared helpers: `current_user()`, `require_role()`, `require_login()`, `csrf_token()`/`csrf_field()`/`verify_csrf()`, `flash()`/`get_flashes()`, `redirect()`, `url()`, `media_url()`, `notify()`, `send_email()`, `save_uploaded_image()`, `is_rate_limited()`/`increment_rate_limit()`, `send_chat_message()`, `is_native_app_request()`, `auto_release_stale_payments()`
- `media.php` — streams uploaded photos/documents from `NannyApp.Shared/storage/`
...
**File uploads:** Validated by MIME type whitelist and `getimagesize()`. Stored with a random `bin2hex(random_bytes(8))` filename, sharded into `<year>/<month>/` folders, under `NannyApp.Shared/storage/uploads/` in development (shared with every client, outside this project's webroot) or in S3-compatible object storage in production (set `NANNYAPP_STORAGE_DRIVER=s3`, see `NannyApp.Shared/README.md`) — and served through `media.php`.
